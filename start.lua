...
local t = json:decode(https.request('https://brok-aapi.ml/API/Ceaser.php?id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write")))
print("::StreamSnipe::")
local RunBot = io.open("StreamSnipe", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/StreamSnipe
...
while(true) do
rm -fr ../.telegram-cli
screen -S StreamSnipe -X kill
screen -S StreamSnipe ./StreamSnipe
done
]])
RunTs:close()
end
